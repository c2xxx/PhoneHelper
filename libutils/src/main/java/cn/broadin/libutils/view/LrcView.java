package cn.broadin.libutils.view;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.Rect;
import android.graphics.Typeface;
import android.os.AsyncTask;
import android.text.TextUtils;
import android.util.AttributeSet;
import android.view.View;
import android.view.animation.LinearInterpolator;
import android.widget.Scroller;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.StringReader;
import java.util.ArrayList;
import java.util.Collections;
import java.util.Iterator;
import java.util.LinkedList;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

import cn.broadin.libutils.Logger;
import cn.broadin.libutils.R;
import cn.broadin.libutils.Utils;


/**
 * liteplayer by loader
 * 显示lrc歌词控件
 */
public class LrcView extends View {
    private static final int SCROLL_TIME = 500;
    private static final String DEFAULT_TEXT = "正在加载歌词...";

    private List<String> mLrcs = new ArrayList<>(); // 存放歌词
    private List<Long> mTimes = new ArrayList<>(); // 存放时间

    private long mNextTime = 0l; // 保存下一句开始的时间

    private int mViewWidth; // view的宽度
    private int mLrcHeight; // lrc界面的高度
    private int mRows;      // 多少行
    private int mCurrentLine = 0; // 当前行
    private long mCurrentTime = 0; // 当前时间
    private int mOffsetY;   // y上的偏移
    private int mMaxScroll; // 最大滑动距离=一行歌词高度+歌词间距

    private float mTextSize; // 普通字体
    private float mTextCurrentSize; // 当前行字体
    private float mDividerHeight; // 行间距

    private Rect mTextBounds;//

    private Paint mNormalPaint; // 常规的字体
    private Paint mCurrentPaint; // 当前歌词的大小

    private Bitmap mBackground;//背景

    private Scroller mScroller;//滚动条

    public LrcView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LrcView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mScroller = new Scroller(context, new LinearInterpolator());
        inflateAttributes(attrs);
    }

    // 初始化操作
    private void inflateAttributes(AttributeSet attrs) {
        // <begin>
        // 解析自定义属性
//        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.Lrc);
        mTextSize = getResources().getDimension(R.dimen.dimen_42sp);
        mTextCurrentSize = getResources().getDimension(R.dimen.dimen_60sp);
        mRows = 8;//行数，出了当前行
        mDividerHeight = getResources().getDimension(R.dimen.dimen_25dp);

        int normalTextColor = getResources().getColor(R.color.white);
//        int currentTextColor = getResources().getColor(R.color.yellow);
        int currentTextColor = Color.parseColor("#47D4E8");
//        int currentTextColor = 0xff00ffde;
//        ta.recycle();
        // </end>

        // 计算lrc面板的高度
        mLrcHeight = (int) (mTextSize * mRows + mTextCurrentSize + mDividerHeight * (mRows + 2));

        mNormalPaint = new Paint();
        mCurrentPaint = new Paint();

        // 初始化paint
        mNormalPaint.setTextSize(mTextSize);
        mNormalPaint.setColor(normalTextColor);
        mNormalPaint.setAntiAlias(true);
        mCurrentPaint.setTextSize(mTextCurrentSize);
        mCurrentPaint.setColor(currentTextColor);
        mCurrentPaint.setAntiAlias(true);

        mTextBounds = new Rect();
        mCurrentPaint.getTextBounds(DEFAULT_TEXT, 0, DEFAULT_TEXT.length(), mTextBounds);
        mMaxScroll = (int) (mTextBounds.height() + mDividerHeight);
//        Utils.setDefaultTypeFace(mCurrentPaint);
//        Utils.setDefaultTypeFace(mNormalPaint);
    }

    /**
     * 设置字体
     *
     * @param face
     */
    public void setTypeFace(Typeface face) {
        mCurrentPaint.setTypeface(face);
        mNormalPaint.setTypeface(face);
    }

//    @Override
//    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
////        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
//        // 重新设置view的高度
//        int measuredHeightSpec = MeasureSpec.makeMeasureSpec(mLrcHeight, MeasureSpec.AT_MOST);
//        super.onMeasure(widthMeasureSpec, measuredHeightSpec);
//    }

    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        super.onSizeChanged(w, h, oldw, oldh);
        // 获取view宽度
        mViewWidth = getMeasuredWidth();
    }

    @Override
    protected void onDraw(Canvas canvas) {
//        Logger.d("onDraw");
        if (mBackground != null) {
//            canvas.drawBitmap(Bitmap.createScaledBitmap(mBackground, mViewWidth, mLrcHeight, true),
//                    new Matrix(), null);
        }

        float centerY = (getMeasuredHeight() + mTextBounds.height()) / 2;
        if (mLrcs.isEmpty() || mTimes.isEmpty()) {
            canvas.drawText(DEFAULT_TEXT,
                    (mViewWidth - mCurrentPaint.measureText(DEFAULT_TEXT)) / 2,
                    centerY, mCurrentPaint);
            return;
        }

        if (mCurrentLine >= mLrcs.size()) {
            return;
        }
        float offsetY = mTextBounds.height() + mDividerHeight;
        String currentLrc = "" + mLrcs.get(mCurrentLine);
        float currentX = (mViewWidth - mCurrentPaint.measureText(currentLrc)) / 2;
        currentX = currentX < 0 ? 0 : currentX;
        // 画当前行
        canvas.drawText(currentLrc, currentX, centerY - mOffsetY, mCurrentPaint);

        int firstLine = mCurrentLine - mRows / 2;
        firstLine = firstLine <= 0 ? 0 : firstLine;
        int lastLine = mCurrentLine + mRows / 2 + 2;
        lastLine = lastLine >= mLrcs.size() - 1 ? mLrcs.size() - 1 : lastLine;

        // 画当前行上面的
        for (int i = mCurrentLine - 1, j = 1; i >= firstLine; i--, j++) {
            String lrc = "" + mLrcs.get(i);
            float x = (mViewWidth - mNormalPaint.measureText(lrc)) / 2;
            canvas.drawText(lrc, x, centerY - j * offsetY - mOffsetY, mNormalPaint);
        }

        // 画当前行下面的
        for (int i = mCurrentLine + 1, j = 1; i <= lastLine; i++, j++) {
            String lrc = "" + mLrcs.get(i);
            float x = (mViewWidth - mNormalPaint.measureText(lrc)) / 2;
            canvas.drawText(lrc, x, centerY + j * offsetY - mOffsetY, mNormalPaint);
        }
    }

    @Override
    public void computeScroll() {
//        Logger.d("computeScroll");
        if (mScroller.computeScrollOffset()) {
            mOffsetY = mScroller.getCurrY();
            if (mScroller.isFinished()) {
                int cur = mScroller.getCurrX();
                mCurrentLine = cur <= 1 ? 0 : cur - 1;
                Long currentTime = -1l;
                if (mCurrentLine < mTimes.size()) {
                    currentTime = mTimes.get(mCurrentLine);//当前时间
                }
                mCurrentTime = currentTime;


                mOffsetY = 0;
            }

            postInvalidate();
        }
    }

    // 解析时间
    private Long parseTime(String time) {
        // 03:02.12
        // 03:02.120
        String[] min = time.split(":");
        String[] sec = min[1].split("\\.");

        long minInt = Long.parseLong(min[0].replaceAll("\\D+", "")
                .replaceAll("\r", "").replaceAll("\n", "").trim());
        long secInt = Long.parseLong(sec[0].replaceAll("\\D+", "")
                .replaceAll("\r", "").replaceAll("\n", "").trim());
        long milInt = 0;
        if (sec.length > 1) {
            milInt = Long.parseLong(sec[1].replaceAll("\\D+", "")
                    .replaceAll("\r", "").replaceAll("\n", "").trim());
        }

        if (("" + milInt).length() == 2) {//可能为3位数，直接写毫秒的，就不用乘了
            milInt = milInt * 10;
        }
        return minInt * 60 * 1000 + secInt * 1000 + milInt;
    }


    // 外部提供方法
    // 传入当前播放时间
    // 如果歌词正在滚动则失效
    public void changeCurrentFix(long time) {
        if (mScroller.isFinished()) {
            changeCurrent(time);
        }
    }

    // 外部提供方法
    // 传入当前播放时间
    // 这个方法不适合频繁调用，间隔应该在500毫秒以上
    public void changeCurrent(long time) {
        time += 200;//提前一点显示
        if (mCurrentTime == time) {
            return;
        }

        // 如果当前时间小于下一句开始的时间
        // 直接return
        if (mCurrentTime < time && mNextTime > time) {
            return;
        }

        // 每次进来都遍历存放的时间
        int timeSize = mTimes.size();
        if (timeSize == 0) {
            reset();
        } else {
            boolean isTimeStart = time < mTimes.get(0);//时间在第一句之前，切换到开头
            int scrollTime = isTimeStart ? 0 : SCROLL_TIME;
            for (int i = 0; i < timeSize; i++) {
                // 发现这个时间大于传进来的时间
                // 那么现在就应该显示这个时间前面的对应的那一行
                // 每次都重新显示，是不是要判断：现在正在显示就不刷新了
                if (mTimes.get(i) > time) {
                    mNextTime = mTimes.get(i);
                    mScroller.abortAnimation();
                    mScroller.startScroll(i, 0, 0, mMaxScroll, scrollTime);
                    postInvalidate();
                    return;
                }
            }
            //当所有歌词都不对时，显示最后一行
            mNextTime += 10 * 60 * 1000;//设置一个很大的时间
            mScroller.abortAnimation();
            mScroller.startScroll(timeSize, 0, 0, mMaxScroll, SCROLL_TIME);
            postInvalidate();
        }
    }


    // 外部提供方法
    // 设置lrc的路径
    public void setLrc(String lrc) {
        reset();
        if (TextUtils.isEmpty(lrc)) {
            mTimes.clear();
            mLrcs.clear();
            postInvalidate();
            return;
        }

        new AsyncTask<String, Void, List<LrcLine>>() {

            @Override
            protected List<LrcLine> doInBackground(String... params) {
                if (params == null || params.length == 0) {
                    return null;
                }
                String lrc = params[0];
                StringReader reader1 = new StringReader(lrc);
                BufferedReader reader = null;
                try {
                    reader = new BufferedReader(reader1);

                    String line;
                    List<LrcLine> lrcLinesPerInFileLine;
                    List<LrcLine> allLines = new LinkedList<>();

                    while (null != (line = reader.readLine())) {
                        lrcLinesPerInFileLine = parseLine(line);
                        if (lrcLinesPerInFileLine == null) {
                            continue;
                        }

                        allLines.addAll(lrcLinesPerInFileLine);
                    }
                    Collections.sort(allLines);
                    for (int i = 0; i < 10; i++) {
                        boolean hasFix = fixLrcs(allLines);//修复前需要排序
                        if (!hasFix) {
                            break;
                        }
                    }
                    Collections.sort(allLines);
                    mTimes.clear();
                    mLrcs.clear();
                    if (allLines.isEmpty()) {
                        return null;
                    }
                    LrcLine lastLine = allLines.get(allLines.size() - 1);
                    if (TextUtils.isEmpty(lastLine.line) || lastLine.line.trim().isEmpty()) {
                        allLines.remove(allLines.size() - 1);
                    }
                    for (LrcLine l : allLines) {
                        mTimes.add(l.time);
                        mLrcs.add(l.line);
                    }
                } catch (Exception e) {
                    mTimes.clear();
                    mLrcs.clear();
                    Logger.e(e);
                } finally {
                    if (reader != null) {
                        try {
                            reader.close();
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
                    if (reader1 != null) {
                        try {
                            reader1.close();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                }
                return null;
            }

            /**
             * 修改歌词列表，
             * @param allLines
             * @return 是否有进行分行，如果一行也没分，说明都是短歌词
             */
            private boolean fixLrcs(List<LrcLine> allLines) {
                if (allLines == null) {
                    return false;
                }
                int lineCount = allLines.size();
                int textMaxWidth = getMeasuredWidth() - 80;//文字最大宽度
                textMaxWidth = Math.max(textMaxWidth, Utils.getDip2Px(1800));//不能小于200
                List<LrcLine> shortLines = new ArrayList<>();//新增的短歌词
                List<Integer> needDelete = new ArrayList<>();//需要删除的

                for (int i = 0, len = allLines.size(); i < len; i++) {
                    LrcLine line = allLines.get(i);
                    //空的或者太短不用分割
                    if (TextUtils.isEmpty(line.line)) {
                        continue;
                    }
                    double lineWidth = mCurrentPaint.measureText(line.line);
                    if (lineWidth < textMaxWidth) {
                        continue;
                    }
                    needDelete.add(i);
                    long endTime = i == (len - 1) ? line.time + 10 * 1000 : allLines.get(i + 1).time;//结束时间
                    long timeLong = endTime - line.time;
                    while (line.line.trim().indexOf("  ") != -1) {//消除两个连续的空格
                        line.line = line.line.trim().replace("  ", " ");
                    }
                    String[] shortStr = line.line.trim().split("，");//1、按中文逗号分
                    if (shortStr.length == 1) {
                        shortStr = line.line.trim().split(",");//2、按英文逗号分
                    }
                    if (shortStr.length == 1) {
                        shortStr = line.line.trim().split(" ");//3、按空格分
                    }
                    if (shortStr.length == 1) {//只有一句没有中断的,按长度分两行
                        int lenStr = line.line.length();
                        int lenA = lenStr / 2;

                        LrcLine lrcLineA = new LrcLine();
                        lrcLineA.time = line.time;
                        lrcLineA.line = line.line.substring(0, lenA);
                        shortLines.add(lrcLineA);

                        LrcLine lrcLineB = new LrcLine();
                        lrcLineB.time = line.time + (timeLong / 2);
                        lrcLineB.line = line.line.substring(lenA, lenStr);
                        shortLines.add(lrcLineB);
                    } else {
                        StringBuilder sb = null;
                        long startTime = line.time;//下一行的开始时间
                        boolean hasCut = false;//到倒数第二个字符串了，如果还没分割过，要分割一次
                        for (int j = 0, lenj = shortStr.length; j < lenj; j++) {
                            if (sb == null) {
                                sb = new StringBuilder();
                            }
                            sb.append(" " + shortStr[j]);
                            double lengCurr = mCurrentPaint.measureText(sb.toString());
                            boolean lengEnough = lengCurr > textMaxWidth * 0.6;
                            if (lengEnough || j == (lenj - 1) || (j == (lenj - 2) && !hasCut)) {
                                LrcLine lrcLine = new LrcLine();
                                lrcLine.time = startTime;
                                startTime = startTime + (long) (timeLong * (lengCurr * 1.0 / lineWidth));
                                lrcLine.line = sb.toString().trim();
                                shortLines.add(lrcLine);
                                sb = null;
                                hasCut = true;
                            }
                        }
                    }
                }

                Iterator<LrcLine> iterator = allLines.iterator();
                int delIndex = 0;

                while (iterator.hasNext()) {
                    iterator.next();
                    if (needDelete.contains(delIndex)) {
                        iterator.remove();
                    }
                    delIndex++;
                }
                allLines.addAll(shortLines);
                return lineCount != allLines.size();
            }

            // 解析每行
            private List<LrcLine> parseLine(String line) {
                List<LrcLine> ret = new LinkedList<>();
                // 设置正则规则
                String reg = "\\[(\\d{2}:\\d{2}\\.{0,1}\\d{0,3})\\]";
                // 编译
                Pattern pattern = Pattern.compile(reg);
                Matcher matcher = pattern.matcher(line);

                // 如果存在匹配项，则执行以下操作
                while (matcher.find()) {
                    long currentTime = 0;
                    String currentContent = null;

                    // 得到这个匹配项中的组数
                    int groupCount = matcher.groupCount();

                    // 得到每个组中内容
                    for (int i = 0; i <= groupCount; i++) {
                        String timeStr = matcher.group(i);
                        if (i == 1) {
                            // 将第二组中的内容设置为当前的一个时间点
                            currentTime = parseTime(timeStr);
                        }
                    }

                    // 得到时间点后的内容
                    String[] content = pattern.split(line);
                    // 输出数组内容
                    for (int i = 0; i < content.length; i++) {
                        if (i == content.length - 1) {
                            // 将内容设置为当前内容
                            currentContent = content[i];
                        }
                    }
                    LrcLine lrcLine = new LrcLine();
                    lrcLine.time = currentTime;
                    lrcLine.line = TextUtils.isEmpty(currentContent) ? "" : currentContent;
                    ret.add(lrcLine);
                }
                return ret;
            }
        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, lrc);
    }


    private void reset() {
        mLrcs.clear();
        mTimes.clear();
        mCurrentLine = 0;
        mCurrentTime = -1;
        mNextTime = 0l;
        postInvalidate();
    }

    // 是否设置歌词
    public boolean hasLrc() {
        return mLrcs != null && !mLrcs.isEmpty();
    }

    // 外部提供方法
    // 设置背景图片
    public void setBackground(Bitmap bmp) {
        mBackground = bmp;
    }

    private static class LrcLine implements Comparable<LrcLine> {
        long time;
        String line;

        @Override
        public int compareTo(LrcLine another) {
            return (int) (time - another.time);
        }
    }
}
